...
def get_y_for_specified_layers_and_nodes(x,number_of_hidden_layers,number_of_nodes_per_hidden_layer,number_particles,number_of_crystals):
    if number_of_hidden_layers==0:
        return -1
    weights={}
    biases={}
    weights["W" + str(1)] = tf.Variable(tf.truncated_normal([number_of_crystals, number_of_nodes_per_hidden_layer], stddev=0.1), dtype=tf.float32)
...
def main(npz_file,number_particles,number_of_hidden_layers,number_of_nodes_per_hidden_layer,output_npz):
    ...
    number_of_crystals=len(x_batch_train[0])

    print('Initializing variables')
    # Making placeholders for the inputdata (x) and the correct output data (y_)
    x = tf.placeholder(dtype=tf.float32, shape=[None, number_of_crystals]) #162=number of crystals in crystal ball detector
    y_ = tf.placeholder(dtype=tf.float32, shape=[None, number_particles+1])

    y=get_y_for_specified_layers_and_nodes(x,number_of_hidden_layers,number_of_nodes_per_hidden_layer,number_particles,number_of_crystals)
    if isinstance(y,int):
    ...
    title='Multiplicity for '+str(number_of_hidden_layers)+'hidden layers and '+str(number_of_nodes_per_hidden_layer)+'nodes per layer.'

    np.savez(output_npz,
             loss_list_train=np.array(loss_list_train), accuracy_list_train=np.array(accuracy_list_train),
             accuracy_list_eval=np.array(accuracy_list_eval), iterations=np.array(iterations),
             trainingtime=Trainingtime,title=title)
...
if __name__ == '__main__':
    main("conv_1try_data_set_XB_up_to_7guns_0dot1_to_10MeV_ca2000000events_iso_digi_sup_90percent.npz", 7, 1, 1024,"plot_data_ord_with_conv_data_up_to_7guns_0dot1_to_10MeV_digi_sup_90_ca2e6events_1layers_1024nodes_5e5its.npz")
